...
const useNavbarSelectLanguage = (): ComputedRef<ResolvedNavbarItem[]> => {
  ...
  return computed<ResolvedNavbarItem[]>(() => {
    ...
    const languageDropdown: ResolvedNavbarItem = {
      ...
      icon: themeLocale.value.selectIcon ?? 'International',
...
const useNavbarRepo = (): ComputedRef<ResolvedNavbarItem[]> => {
  ...
  const repoIcon = computed(() => {
    ...
    if (themeLocale.value.repoIcon) return themeLocale.value.repoIcon
    if (repoType.value === 'GitHub') return 'Github'
    if (repoType.value === 'GitLab') return 'Gitlab'
    if (repoType.value) return 'FolderCode'
    return repoType.value
  })
